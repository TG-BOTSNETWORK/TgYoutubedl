...
def save_on_off(user_id, status):
    column_name = f"{inspect.currentframe().f_code.co_name.split('_')[1]}_download_status"
    status = status if status is not None else False
    query = f"INSERT INTO download_status (user_id, {column_name}) VALUES (%s, %s) ON CONFLICT (user_id) DO UPDATE SET {column_name} = %s RETURNING {column_name};"
